---
_slug: research-2026-07-21-研究報告-agent-state-recovery-idempotency-crash-consistency-in-long-horizon-workflows
_vault_path: research/2026-07-21-研究報告-agent-state-recovery-idempotency-crash-consistency-in-long-horizon-workflows.md
tags:
- research
- knowledge
- ai-agent
created: '2026-07-21'
version: 1
source_report: 2026-07-21-agent-durability-checkpoint-resume-2026.md
source_url: ''
type: research
fingerprint: step, key, agent, idempotency, durable-agents, durable, llm, sqlite,
  firn, worker
title: 研究報告：Agent State Recovery, Idempotency & Crash-Consistency in Long-Horizon
  Workflows
status: seedling
updated: '2026-07-21'
---
...
### 核心觀念
**問題**：**為什麼這個問題重要？** 長時執行（long-horizon）AI agent workflow 是 2026 年 agent 系統最大的 reliability gap。naive loop `while not done: step = llm(); tool(step)` 在 notebook 沒事，但在 production 一定爆炸： - **OOM kill**：20 分鐘的 run 到第 12 分鐘被 systemd OOM-kill → 從頭開始 → 燒光 token budget - **重複副作用**：retry 把同一封信寄兩次、Stripe 扣兩次款 - **Poi…

**洞見**：**理論影響**：把「agent reliability」從「prompt engineering + retry」推進到「formal durability layer」。agent 變成可以**長期跑**而不會燒光預算。 **實務影響**： 1. **Cost ceiling 被打破**：沒 durability 就沒人敢讓 agent 跑 30 分鐘以上（怕 crash 浪費 token）；有 durability 後 24-hour autonomous run 變可行 2. **HITL 終於能 scale**：approval gate 的「parking lot」是 durabl…
...
- 原始報告：2026-07-21-agent-durability-checkpoint-resume-2026.md
- 類型：
- 連結：
