---
_slug: 40-Resources-_mixed-explorations-2026-06-01-AxonFlow---Pomerium---Agentic-Governance-Tools-Exploration
_vault_path: 40-Resources/_mixed/explorations/2026-06-01-AxonFlow---Pomerium---Agentic-Governance-Tools-Exploration.md
...
### Computer Use Governor 實作模式

AxonFlow 的 `ComputerUseGovernor` adapter 是目前最完整的 tool-call 治理範本：

```python
# Pre-execution: check_tool_use() — blocking patterns 優先於 API call
governor.check_tool_use({"name": "bash", "input": {"command": "..."}})
# → 直接在本地比對 10 條預設 patterns（rm -rf /, curl|bash, cat ~/.ssh/ 等）
# → 拒絕時不回 API，直接 block，延遲 0ms

# Post-execution: check_result() — PII scan
governor.check_result("bash", output)
# → 回傳 {allowed, block_reason, redacted_result, policies_evaluated}
# → PII detected 時 redacted_result 有值，allowed 仍為 True（redact 模式預設）
```

**10 條預設 blocking patterns**（local regex，無需網路）：
...
- `cat .env` — secret exfiltration
- `chmod 777` — overly permissive
- `>/dev/sd*` — direct disk write

**對 Talos governance 的直接價值**：
- 可以直接取用這 10 patterns 作為初始 blocklist
- `connector_type` 命名慣例（`computer_use.bash`）→ 可擴展至 `hermes.{tool_name}`
- Observe mode (`PII_ACTION=log`) 讓上線前先看 audit trail，不直接 block

### Claude Agent SDK Integration Pattern

TypeScript SDK 的 `mcpCheckInput()` / `mcpCheckOutput()` pattern：
- 4-10ms overhead（對 50-500ms 的 tool call 可忽略）
- Audit write 完全 non-blocking（async）
- connector type convention `agent_sdk.{tool_name}` 可直接映射到 Talos 的 `hermes.{tool_name}`

### HITL Approval Gates

Enterprise feature，概念驗證價值高：
- 危險 tool 執行前需要 human approval
- Configurable expiry + pending limits + auto-abort on expiration
- 適合用於 `delegate_task`（危險操作）、`patch`（生產環境）、`rm`（資料刪除）
