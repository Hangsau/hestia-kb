---
_slug: 40-Resources-_mixed-explorations-2026-05-29-2026-05-29-NERDs---Memex-RL----實體中心記憶-vs-索引體驗記憶
_vault_path: 40-Resources/_mixed/explorations/2026-05-29-2026-05-29-NERDs---Memex-RL----實體中心記憶-vs-索引體驗記憶.md
...
## Memex(RL) — Abstract Deep Read（延續）

**Source**: arXiv:2603.04257 (submitted 2026-03-04)
**Link**: https://arxiv.org/abs/2603.04257
**Fetched**: 未經 sanitizer（HTML 版為站點導航，直接取 abstract page）

### 定量數據（原文）

| Task | MemexRL Agent | Baseline |
|------|-------------|----------|
| Long-horizon task success | ↑ improved | baseline |
| Working context size | significantly smaller | full trajectory |

理論保證：
- Bounded dereferencing：decision quality 有界，不隨 history 線性退化
- Effective in-context computation 有界

### MemexRL Reward Shaping 細節

RL 優化四維決策：
1. **What to summarize** — 壓縮維度
2. **What to archive** — 存儲維度  
3. **How to index it** — 組織維度
4. **When to retrieve it** — 讀取維度（關鍵缺口）

Reward signal: context budget efficiency + indexed memory usage →鼓勵「壓縮而不丟失」而非「直接丟棄」

### 與 NERDs 的互補性

| | NERDs | Memex |
|---|---|---|
| 生成時機 | Preprocessing（建好後靜態） | On-demand + RL-learned |
| 索引單元 | Entity document | Index → dereference |
| 壓縮策略 | 預生成 entity，有結構但靜態 | RL 動態學習壓縮/索引/讀取策略 |
| 適用場景 | 明確 entity 的長文本（小説、代碼） | 開放式長程任務 |

**互補的關鍵**：NERDs 擅長結構化長文本的 entity 追蹤（西遊記角色網絡），Memex 擅長動態决策「什麼時候該 retrieve」。两者可叠加：NERDs 做 entity 建構，MemexRL 學習何時dereference。

### 直接應用於 Hermes

**WS-035 drift penalty → bounded dereferencing**：
- Heartbeat learning 的 distillate 決策目前是被動的（feed new → distill）
- MemexRL 框架提供主動決策機制：given current context budget, should I distill now or wait?
- 建議：heartbeat_learning.py 的 `should_distill()` 加入簡單的 staleness window + context budget check（不用 RL，用啟發式）

### Lead Update

- ❌ `https://arxiv.org/html/2603.04257v1` — HTML 版為站點導航，無論文內容 → skip
- ✅ `https://arxiv.org/abs/2603.04257` — abstract page 內容完整，直接用
