---
_slug: research-2026-07-03-研究報告-opentelemetry-genai-semantic-conventions-agent-執行-trace-的形式化-2026-h1
_vault_path: research/2026-07-03-研究報告-opentelemetry-genai-semantic-conventions-agent-執行-trace-的形式化-2026-h1.md
tags:
- research
- knowledge
- ai-agent
created: '2026-07-03'
version: 1
source_report: 2026-07-03-otel-genai-semantic-conventions-agent-tracing.md
source_url: ''
type: research
fingerprint: otel, agent, trace, genai, cache, mcp, gen, attribute, langfuse, firn
title: 研究報告：OpenTelemetry GenAI Semantic Conventions — Agent 執行 Trace 的形式化（2026 H1）
status: seedling
updated: '2026-07-03'
---
...
### 核心觀念
**問題**：「Agent 執行 trace 形式化」這個問題在 2024 還只是少數 production 團隊在苦惱——他們寫了幾千行 custom log，自己拼 JSONL、想辦法接 Jaeger、寫 Grafana dashboard。**2026 H1 這個問題從「各家自幹」變成「業界正在收斂到一份官方規範」**，而且這份規範不是 vendor-specific 的（不是 LangChain 的、不是 OpenAI 的），是 **OpenTelemetry 官方 GenAI semantic conventions**。 為什麼這件事突然變得這麼重要？三個驅動力： 1. **Multi-agen…

**洞見**：1. **「跨 provider trace 統一查詢」變得可行**。之前你要看 Claude 的 token cost 跟 GPT-4 的 token cost 對比，得自己 parse 兩家 SDK 的 log。OTel GenAI 規範下，`gen_ai.provider.name` 跟 `gen_ai.usage.input_tokens` 是固定 attribute，直接 group-by provider 算 cost 就行。 2. **Agent observability 從「框架特性」變成「infra 基礎設施」**。OTel 已經是所有雲端 observability 的事…
...
- 原始報告：2026-07-03-otel-genai-semantic-conventions-agent-tracing.md
- 類型：
- 連結：
