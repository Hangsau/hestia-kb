...
_raw_fm: '
  ...
  fingerprint: [agent, cache, caching, hermes, llm, loop, memory, prompt, pts, tool]
...
**標籤**: #agent-architecture #memory-design #prompt-caching #tool-use
...
- **Beads: A coding agent memory system** (Steve Yegge, 19 pts) — Medium Cloudflare-walled，無法讀取。HN 討論只有 1 則 comment。GitHub 可能有 repo（未確認）。
- **Two things LLM coding agents are still bad at** (345 pts) — kix.dev，下次可讀。
- **Expensively Quadratic: The LLM Agent Cost Curve** (131 pts) — 與 prompt caching 高度互補。
- **Design Patterns for Securing LLM Agents Against Prompt Injections** (Simon Willison, 110 pts) — 安全維度，遲早要面對。
