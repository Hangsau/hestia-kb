---
_slug: 40-Resources-_mixed-research-2026-05-28-研究報告-self-improving-agents-從外部工具調用到內部策略演化
_vault_path: 40-Resources/_mixed/research/2026-05-28-研究報告-self-improving-agents-從外部工具調用到內部策略演化.md
tags:
- research
- knowledge
- ai-agent
created: '2026-05-28'
version: 1
source_report: 2026-05-28-self-improving-agents-external-scaffolding.md
source_url: null
type: research
fingerprint: arxiv, agent, skill, https, core, medium, search, bes, llm, firn
title: 研究報告：Self-Improving Agents — 從外部工具調用到內部策略演化
updated: '2026-06-15'
status: budding
---
...
## Version 1 — 2026-05-28

### 核心觀念
**問題**：LLM-based agent 在複雜任務中失敗的模式高度情境化——同一個 agent 在專案 A 失敗的原因，在專案 B 完全不會重現。傳統做法是微調模型，但這需要大量計算資源且無法針對單次失敗做快速修正。 2026 年 5 月底的最新趨勢是：**從「改善模型」轉向「改善 agent 的外部支架（scaffold）」**——包括 skill library、memory architecture、search strategy、和 self-correction protocol。這些方法不需要改變模型權重，可以在 inference time 運作。

**洞見**：**對 AI agent 領域的影響：** 1. **Inference-time self-improvement 正在成熟**：不再需要完整 training run，agent 可以在 session 中即時改善策略。這降低了一般開發者的進入門檻。 2. **去中心化記憶是 multi-agent 的瓶頸被解決的信號**：DecentMem 的理論保證，讓我們有信心建構真正分散的多 agent 系統，不再需要中心化的記憶協調者。 3. **Skill 會成為一等公民**：就像軟體開發從「monolithic」走向「microservices」，agent 技能系統會走向可組合、可測試、可…
...
- 原始報告：2026-05-28-self-improving-agents-external-scaffolding.md
- 類型：
- 連結：
